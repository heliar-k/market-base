...
class TestPolymarketCrypto:
    """_polymarket：crypto 过滤 + hit-price 分组排序 + 历史 7 日变化。"""

    @staticmethod
    def _ev(eid, title, series, category, markets, volume=1.0):
        return {
            "id": eid,
            "slug": eid,
            "title": title,
            ...
            "events": [
                cls._ev(
                    ...
                    [
                        ...
                        {
                            "id": 112,
                            "question": "Will Bitcoin dip to $60,000 in 2026?",
                            "prob_yes": 0.8,
                            "volume24hr": 20.0,
                        },
                    ],
                ...
                cls._ev(
                    ...
                    [
                        {
                            "id": 211,
                            "question": "Will Ethereum reach $6,000 in 2026?",
                            "prob_yes": 0.5,
                            "volume24hr": 1.0,
                        }
                    ],
    ...
    def test_missing_dir_returns_none(self, tmp_path, monkeypatch):
        from src.assets_analysis import _polymarket

        self._patch(monkeypatch, tmp_path)
        assert _polymarket() is None

    def test_crypto_filter_grouping_and_sort(self, tmp_path, monkeypatch):
        from src.assets_analysis import _polymarket

        self._write(tmp_path, "20260915.json", self._snap())
        self._patch(monkeypatch, tmp_path)

        out = _polymarket()
        assert out["as_of"] == "2099-01-01"
        # 分组顺序 BTC → ETH；fed 事件不出现
        assert [g["name"] for g in out["groups"]] == ["BTC", "ETH"]
        btc = out["groups"][0]["events"][0]
        # 组内市场按 prob_yes 降序；label 剥掉 "Will "/"?"
        assert [m["prob_yes"] for m in btc["markets"]] == [0.8, 0.3]
        assert btc["markets"][0]["label"] == "Bitcoin dip to $60,000 in 2026"
        assert all(m["chg7d"] is None for m in btc["markets"])  # 无 history.csv

    def test_chg7d_from_history(self, tmp_path, monkeypatch):
        from src.assets_analysis import _polymarket

        self._write(tmp_path, "20260915.json", self._snap())
        pd.DataFrame(
            {
                "date": ["2026-09-01", "2026-09-08", "2026-09-15"],
                "111": [0.2, None, 0.5],  # 111：7 日 +30pp；缺失日剔除
                "999": [0.1, 0.2, 0.3],  # 非 crypto 市场 → 不提取
            }
        ).to_csv(tmp_path / "data" / "polymarket" / "history.csv", index=False)
        self._patch(monkeypatch, tmp_path)

        out = _polymarket()
        btc = out["groups"][0]["events"][0]
        by_id = {str(m["id"]): m for m in btc["markets"]}
        assert by_id["111"]["chg7d"] == 30.0  # 0.2 → 0.5
        assert by_id["112"]["chg7d"] is None  # 无历史列

    def test_corrupt_json_falls_back_to_older(self, tmp_path, monkeypatch):
        from src.assets_analysis import _polymarket

        self._write(tmp_path, "20260914.json", self._snap())
        (tmp_path / "data" / "polymarket" / "20260915.json").write_text(
    ...
    def test_hit_price_series_filter(self, tmp_path, monkeypatch):
        """非 hit-price 系列（multi-strikes 等）不进价位阶梯；全是则回退全部。"""
        from src.assets_analysis import _polymarket
        ...
        snap = {
            ...
            "events": [
                self._ev(
                    "w",
                    "Bitcoin above ___ on September 15?",
                    "btc-multi-strikes-weekly",
                    "crypto",
                    [{"id": 1, "question": "above $68K?", "prob_yes": 0.99}],
                )
            ],
        }
        self._write(tmp_path, "20260915.json", snap)
        self._patch(monkeypatch, tmp_path)
        # 无 hit-price 事件 → 回退全部 crypto 事件（分组仍可用）
        out = _polymarket()
        assert out["groups"][0]["name"] == "BTC"
        assert out["groups"][0]["events"][0]["title"].startswith("Bitcoin above")
